
export const starterNavAnim = {
  mounted() {
    this.$nextTick(() => {
      const tl = new TimelineMax();
      tl.set(this.$el, {
        autoAlpha: 1,
        x: this.$el.offsetWidth
      });
    });
  }
};

export const afterToggle = {
  watch: {
    open(open) {
      const tl = new TimelineMax(),
        C = document.querySelectorAll(".navbar__list, .Social"),
        B = document.querySelectorAll(
          ".case-holder__img, .case-holder__bar, .case-holder__group"
        ),
        dX = open ? 0 : this.$el.offsetWidth;

      if (!!open) {
        tl.set(C, { xPercent: -40, autoAlpha: 0 }).set(B, {
          xPercent: 30,
          autoAlpha: 0
        });
      }

      tl.to(this.$el, 1.25, {
        x: dX,
        ease: blendEases(Power1.easeIn, Power3.easeOut, false),
        onComplete() {
          if (!this.open) {
            afterToggle_prop();
          }
        }
      });

      setTimeout(() => {
        open
          ? document.documentElement.classList.add("hide-scroll")
          : document.documentElement.classList.remove("hide-scroll");
      }, 800);

      function blendEases(startEase, endEase, blender) {
        blender = blender || Power4.easeInOut;
        return new Ease(function(v) {
          var b = blender.getRatio(v);
          return startEase.getRatio(v) * (1 - b) + endEase.getRatio(v) * b;
        });
      }

      function afterToggle_prop() {
        tl.add("leftSide").staggerTo(
          C,
          0.75,
          { xPercent: 0, autoAlpha: 1, ease: Power2.easeOut },
          0.1
        );
        tl.staggerTo(
          B,
          0.75,
          { xPercent: 5, autoAlpha: 1, ease: Power2.easeInOut },
          0.1,
          "leftSide+=.3"
        );

        return tl;
      }
    }
  }
};
